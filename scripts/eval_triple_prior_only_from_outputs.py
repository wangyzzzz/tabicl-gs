...
from tabicl_gs.models.group_shared_gate import _fit_simplex_prior_weights
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate BayesB+GBLUP+RKHS triple-prior-only from existing triple-prior caches.")
    parser.add_argument("--triple-root", required=True, help="Trait root containing tabicl_tabicl_triple_prior/")
...
def _load_prior_prediction(prior_cache: Path, stem: str, fallback_path: Path) -> np.ndarray:
    npy_path = prior_cache / f"{stem}.npy"
    if npy_path.exists():
        return _load_npy(npy_path)
    if fallback_path.suffix == ".npy":
        return _load_npy(fallback_path)
    return np.loadtxt(fallback_path, delimiter=",", dtype=np.float32).reshape(-1)
...
def _estimate_prior_weights_from_fold1_oof(triple_dir: Path, base_config: dict) -> np.ndarray:
    fold1_prior = triple_dir / "fold_1" / "prior_cache"
    ...
    for inner_id, (_, valid_idx) in enumerate(inner_splits, start=1):
        inner_dir = inner_dirs[inner_id - 1]
...
def _load_fold_predictions(
    ...
    rkhs_test = _load_prior_prediction(
        prior_cache,
        "rkhs_test",
        prior_cache / "rkhs_outer" / "eval_fit" / "predictions.csv",
    )
    return y_true, bayesb_test, gblup_test, rkhs_test


def evaluate_triple_prior_only(
    dual_root: Path,
    dataset: str,
    trait_col: str,
) -> tuple[pd.DataFrame, dict[str, object]]:
    triple_dir = dual_root / "tabicl_tabicl_triple_prior"
    base_config = _build_base_config(dataset, trait_col)
    weights = _estimate_prior_weights_from_fold1_oof(triple_dir, base_config)
    rows = []
    for fold_id in range(1, 6):
        fold_dir = triple_dir / f"fold_{fold_id}"
        y_true, bayesb_test, gblup_test, rkhs_test = _load_fold_predictions(
            fold_dir,
            base_config,
            fold_id,
        )
        pred = (
            float(weights[0]) * bayesb_test
            + float(weights[1]) * gblup_test
            + float(weights[2]) * rkhs_test
        ).astype(np.float32)
        metric = regression_metrics(y_true, pred)
        rows.append(
            {
                "fold": int(fold_id),
                "w_bayesb": float(weights[0]),
                "w_gblup": float(weights[1]),
                "w_rkhs": float(weights[2]),
    ...
    metadata: dict[str, object] = {
        ...
        "weight_source": "fold1_inner_val_oof_prior",
        "weight_source_dir": str(triple_dir / "fold_1" / "prior_cache"),
    }
...
def main() -> None:
    ...
    fold_df, metadata = evaluate_triple_prior_only(
        dual_root=triple_root,
        dataset=args.dataset,
        trait_col=args.trait_col,
    )
    out_dir = triple_root / "prior_only_triple"
