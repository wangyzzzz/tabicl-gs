...
OUT_ROOT="outputs/5.4-sample_size-decoupled"
...
run_trait() {
  ...
  CUDA_VISIBLE_DEVICES="${gpu}" PYTHONUNBUFFERED=1 PYTHONPATH=src "${PYTHON_BIN}" \
    scripts/run_54_sample_size_decoupled_trait.py \
    --dataset "${dataset}" \
    --trait-col "${trait}" \
...
PYTHONPATH=src "${PYTHON_BIN}" scripts/summarize_54_sample_size_decoupled.py \
  --sample-root "${OUT_ROOT}" \
  --full-main-results-csv "${FULL_MAIN_RESULTS}" \
  --output-csv "${OUT_ROOT}/sample_size_main_results.csv" \
  --output-json "${OUT_ROOT}/sample_size_main_results.json" | tee "${LOG_DIR}/summary.log"
