...
from tabicl_gs.pipeline.dual_prior_fold_search import run_dual_prior_fixed_block_with_frozen_gate_on_fold


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run fixed-block evaluation with a frozen fold1 group gate.")
    parser.add_argument("--config", required=True)
    parser.add_argument("--output-root", required=True)
    parser.add_argument("--trait-col", default=None)
    parser.add_argument("--group-size", type=int, required=True)
    parser.add_argument("--gate-summary-path", required=True)
...
def main() -> None:
    ...
    for fold_id in args.fold_ids:
        row = run_dual_prior_fixed_block_with_frozen_gate_on_fold(
            ...
            gate_summary_path=args.gate_summary_path,
